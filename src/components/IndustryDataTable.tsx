...
export default function IndustryDataTable({
  ...
  const handleContactClick = useCallback((aiUseCase: string, industry: string) => {
    const subject = aiUseCase
      ? `Interest in: ${aiUseCase}${industry ? ` (${industry})` : ""}`
  ...
  const tableData = useMemo<IndustryData[]>(() => {
    if (aiMode && aiResults.length > 0) return aiResults.map(r => r.item);
    return filteredData;
          ...
          <>
            <Box sx={{ display: "flex", gap: 1, alignItems: "center", mt: 1 }}>
              <TextField
                size="small"
                fullWidth
                placeholder="Describe what you're looking for — e.g. 'AI use cases in healthcare for patient risk prediction'"
                value={aiQuery}
            ...
            {aiError && (
              <Alert severity="error" sx={{ mt: 0.75, py: 0.25, fontSize: "0.75rem" }}>{aiError}</Alert>
            )}
        ...
        {!aiEnabled && (
          <Box sx={{ mt: 1 }}>
            <TextField
              size="small"
              fullWidth
              placeholder="Search across all columns — e.g. 'healthcare', 'supply chain', 'forecasting'"
              value={globalFilter}
